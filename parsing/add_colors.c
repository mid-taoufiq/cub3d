...
/*   Updated: 2025/08/21 14:27:35 by tibarike         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ceiling_part(t_wall *wall_dim, char *line)
{
	char	**surface_split;
	...
	if (ft_strsearch(line, "C "))
	{
		...
		if (arg_counter(surface_split) != 2)
			return (free_splits(surface_split),
				write(2, "wrong file arguments\n", 21), 0);
		if (!add_colors2(surface_split, wall_dim->ceiling))
			return (0);
		wall_dim->f_filled = 1;
		return (2);
	}
...
int	add_colors(t_wall *wall_dim, char *line)
{
	...
	if (ft_strsearch(line, "F "))
	{
		...
		if (arg_counter(surface_split) != 2)
			return (free_splits(surface_split),
				write(2, "wrong file arguments\n", 21), 0);
		if (!add_colors2(surface_split, wall_dim->floor))
			return (0);
		wall_dim->f_filled = 1;
		return (2);
	}
	return (ceiling_part(wall_dim, line));
}
