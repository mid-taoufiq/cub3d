...
/*   Updated: 2025/08/25 12:44:58 by tibarike         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*extract_line(char **buffer)
{
	...
	if (index >= 0)
	{
		line = ft_substr2(*buffer, 0, index);
		tmp = ft_substr2(*buffer, index + 1, ft_strlen(*buffer) - index - 1);
		if (!line || !tmp)
	...
	{
		line = ft_substr2(*buffer, 0, ft_strlen(*buffer));
		free(*buffer);
		*buffer = NULL;
		if (!line)
			return (NULL);
